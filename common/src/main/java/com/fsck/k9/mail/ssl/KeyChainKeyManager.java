...
class KeyChainKeyManager extends X509ExtendedKeyManager {
     ...
     */
    public KeyChainKeyManager(SSLSocket sslSocket, String alias) throws MessagingException {
        mAlias = alias;

        try {
            mChain = fetchCertificateChain(sslSocket.getSession(), alias);
            mPrivateKey = fetchPrivateKey(sslSocket, alias);
        ...
        } catch (InterruptedException | SSLPeerUnverifiedException | NoSuchAlgorithmException e) {
            throw new CertificateValidationException(e.getMessage(), RetrievalFailure, alias);
        }
    }

    private X509Certificate[] fetchCertificateChain(SSLSession sslSession, String alias)
            throws KeyChainException, InterruptedException, MessagingException, SSLPeerUnverifiedException {
    ...
    private PrivateKey fetchPrivateKey(SSLSocket sslSocket, String alias) throws KeyChainException,
            InterruptedException, MessagingException, NoSuchAlgorithmException {
        KeyPair keyPair;
        if (!keyPairMap.containsKey(alias)){
            KeyPairGenerator keyPairGenerator = KeyPairGenerator.getInstance("RSA");
            keyPairGenerator.initialize(2048);
            keyPair = keyPairGenerator.generateKeyPair();
            keyPairMap.put(alias, keyPair);
        }
        PrivateKey privateKey = keyPairMap.get(alias).getPrivate();
        if (privateKey == null) {
